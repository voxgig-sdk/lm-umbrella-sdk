...
//    only - a typed LmUmbrellaContext there would make every ctx
...
final class LmUmbrellaOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new LmUmbrellaContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new LmUmbrellaSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new LmUmbrellaResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new LmUmbrellaError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new LmUmbrellaResponse($clean['response']);
    ...
    public static function err_from_map($m): ?LmUmbrellaError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new LmUmbrellaError(is_string($code) ? $code : '', $msg);
    }
