...
class FlattenedPermissionEntity
  ...
  def make
    opts = @_entopts.dup
    FlattenedPermissionEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = LmUmbrellaHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [FlattenedPermission, Hash] the current FlattenedPermission data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = LmUmbrellaHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of FlattenedPermission fields)
  def match_get
  ...
  #
  # @param reqmatch [FlattenedPermissionLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.FlattenedPermission.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [FlattenedPermission, Hash] the loaded FlattenedPermission; raises LmUmbrellaError on failure
  ...
  #
  # @param reqmatch [FlattenedPermissionListMatch, Hash, nil] match filter (any subset of
  #   FlattenedPermission fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<FlattenedPermission>, Array] the matching FlattenedPermission items; raises LmUmbrellaError on failure
  ...
  #
  # @param reqdata [FlattenedPermissionCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [FlattenedPermission, Hash] the created FlattenedPermission; raises LmUmbrellaError on failure
