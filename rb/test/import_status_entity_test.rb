...
class ImportStatusEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.ImportStatus(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.ImportStatus(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = import_status_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create", "list"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "import_status." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    # CREATE
    import_status_ref01_ent = client.ImportStatus(nil)
...
def import_status_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "import_status", "ImportStatusTestData.json")
