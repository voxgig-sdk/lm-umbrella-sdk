...
class PaginatedPermissionListEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set LM_UMBRELLA_TEST_PAGINATED_PERMISSION_LIST_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    paginated_permission_list_ref01_data = Helpers.to_map(paginated_permission_list_ref01_data_result.respond_to?(:data_get) ? paginated_permission_list_ref01_data_result.data_get : paginated_permission_list_ref01_data_result)
...
def paginated_permission_list_basic_setup(extra)
  ...
  entid_env_raw = ENV["LM_UMBRELLA_TEST_PAGINATED_PERMISSION_LIST_ENTID"]
