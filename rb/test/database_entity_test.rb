...
class DatabaseEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set LM_UMBRELLA_TEST_DATABASE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    database_ref01_data = nil
    if database_ref01_data_raw.length > 0
...
def database_basic_setup(extra)
  ...
  entid_env_raw = ENV["LM_UMBRELLA_TEST_DATABASE_ENTID"]
