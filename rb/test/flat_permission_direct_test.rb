...
class FlatPermissionDirectTest < Minitest::Test
  def test_direct_load_flat_permission
    setup = flat_permission_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-flat_permission", setup[:live] ? "live" : "unit")
    if _should_skip
...
def flat_permission_direct_setup(mockres)
