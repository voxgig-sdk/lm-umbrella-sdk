...
class FlattenedPermissionDirectTest < Minitest::Test
  def test_direct_list_flattened_permission
    setup = flattened_permission_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-flattened_permission", setup[:live] ? "live" : "unit")
    if _should_skip
  ...
  def test_direct_load_flattened_permission
    setup = flattened_permission_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-flattened_permission", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
...
def flattened_permission_direct_setup(mockres)
