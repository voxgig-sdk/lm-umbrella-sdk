...
module LmUmbrellaUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = LmUmbrellaResponse.new({})
    ...
    if fetch_err
      ...
      response = LmUmbrellaResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = LmUmbrellaResponse.new(fetched)
    else
