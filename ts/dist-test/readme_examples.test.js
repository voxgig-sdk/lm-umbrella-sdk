...
const TEST_SEED = { "entity": { "database": { "test01": { "id": "test01" } }, "flat_permission": { "test01": { "id": "test01" } }, "flattened_permission": { "test01": { "id": "test01" } }, "import_status": { "test01": { "id": "test01" } }, "metadata": { "test01": { "id": "test01" } }, "paginated_permission_list": { "test01": { "id": "test01" } }, "permission": { "test01": { "id": "test01" } }, "permission_database": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+LmUmbrellaSDK\b/.test(code) ||
        /\bLmUmbrellaSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+LmUmbrellaSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/LmUmbrellaSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
