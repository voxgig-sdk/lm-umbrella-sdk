...
(0, node_test_1.describe)('MetadataDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-metadata', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-metadata', setup.live))
            return;
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['LM_UMBRELLA_TEST_METADATA_ENTID'];
