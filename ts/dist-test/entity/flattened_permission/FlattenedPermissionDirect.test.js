...
(0, node_test_1.describe)('FlattenedPermissionDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-flattened_permission', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-flattened_permission', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["database01"]))
            return;
        ...
        if (setup.live) {
            ...
            const candidateId = listArr[0]?.database_id ?? listArr[0]?.id;
            if (null == candidateId) {
                throw new Error('Live load blocked: discovery returned no usable identity');
            }
            params.database_id = candidateId;
    ...
    (0, node_test_1.test)('direct-list-flattened_permission', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-flattened_permission', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["database01"]))
            return;
        const { client, calls } = setup;
...
function liveScenariosActive() { return false && process.env.LM_UMBRELLA_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.LmUmbrellaSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
            apikey: env.LM_UMBRELLA_APIKEY,
        }));
        let idmap = env['LM_UMBRELLA_TEST_FLATTENED_PERMISSION_ENTID'];
...
//# sourceMappingURL=FlattenedPermissionDirect.test.js.map