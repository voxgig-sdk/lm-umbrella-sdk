...
describe('FlattenedPermissionDirect', async () => {
  ...
  test('direct-load-flattened_permission', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-flattened_permission', setup.live)) return
    if (skipIfMissingIds(t, setup, ["database01"])) return
  ...
  test('direct-list-flattened_permission', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-flattened_permission', setup.live)) return
    if (skipIfMissingIds(t, setup, ["database01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['LM_UMBRELLA_TEST_FLATTENED_PERMISSION_ENTID']
