...
describe('FlatPermissionDirect', async () => {
  ...
  test('direct-load-flat_permission', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-flat_permission', setup.live)) return
    if (skipIfMissingIds(t, setup, ["database_id01","id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['LM_UMBRELLA_TEST_FLAT_PERMISSION_ENTID']
