...
describe('ImportStatusDirect', async () => {
  ...
  test('direct-list-import_status', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-import_status', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['LM_UMBRELLA_TEST_IMPORT_STATUS_ENTID']
