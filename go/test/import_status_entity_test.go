...
func TestImportStatusEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.ImportStatus(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.ImportStatus(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set LMUMBRELLA_TEST_IMPORT_STATUS_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		importStatusRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath([]any{"new", "import_status"}, setup.data), "import_status_ref01"))
		...
		importStatusRef01Data = core.ToMapAny(importStatusRef01DataResult)
		...
		importStatusRef01List, importStatusRef01ListOk := importStatusRef01ListResult.([]any)
		if !importStatusRef01ListOk {
			t.Fatalf("expected list result to be an array, got %T", importStatusRef01ListResult)
		}

		foundItem := vs.Select(entityListToData(importStatusRef01List), map[string]any{"id": importStatusRef01Data["id"]})
...
func import_statusBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("LMUMBRELLA_TEST_IMPORT_STATUS_ENTID")
	...
	idmapResolved := core.ToMapAny(env["LMUMBRELLA_TEST_IMPORT_STATUS_ENTID"])
