...
func TestMetadataDirect(t *testing.T) {
	...
	t.Run("direct-load-metadata", func(t *testing.T) {
		setup := metadataDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-metadata", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
func metadataDirectSetup(mockres any) *metadataDirectSetupResult {
	...
	env := envOverride(map[string]any{
		"LM_UMBRELLA_TEST_METADATA_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["LM_UMBRELLA_TEST_METADATA_ENTID"]; ok {
