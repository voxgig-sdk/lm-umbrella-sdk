...
func TestFlatPermissionDirect(t *testing.T) {
	t.Run("direct-load-flat_permission", func(t *testing.T) {
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-flat_permission", _mode); _shouldSkip {
			if _reason == "" {
...
type flat_permissionDirectSetupResult struct {
...
func flat_permissionDirectSetup(mockres any) *flat_permissionDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"LM_UMBRELLA_TEST_FLAT_PERMISSION_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["LM_UMBRELLA_TEST_FLAT_PERMISSION_ENTID"]; ok {
		...
		return &flat_permissionDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &flat_permissionDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
