...
import (
	...
	omni "github.com/voxgig-sdk/lm-umbrella-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.LmUmbrellaError); is {
				out := map[string]any{
					"name":    "LmUmbrellaError",
...
func omniCtx(arg any, client *sdk.LmUmbrellaSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.LmUmbrellaSDK); is {
			client = live
