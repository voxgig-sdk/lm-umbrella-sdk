...
class TestImportStatusEntity:

    def test_should_create_instance(self):
        testsdk = LmUmbrellaSDK.test(None, None)
        ent = testsdk.ImportStatus(None)
    ...
    def test_should_stream(self):
        ...
        seen = list(base.ImportStatus(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.ImportStatus(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _import_status_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create", "list"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "import_status." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set LM_UMBRELLA_TEST_IMPORT_STATUS_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        import_status_ref01_ent = client.ImportStatus(None)
        ...
        import_status_ref01_data = helpers.to_map(runner.entity_data(import_status_ref01_ent.create(import_status_ref01_data, None)))
        assert import_status_ref01_data is not None
        ...
        import_status_ref01_list_result = import_status_ref01_ent.list(import_status_ref01_match, None)
        assert isinstance(import_status_ref01_list_result, list)



def _import_status_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/import_status/ImportStatusTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("LM_UMBRELLA_TEST_IMPORT_STATUS_ENTID"))
