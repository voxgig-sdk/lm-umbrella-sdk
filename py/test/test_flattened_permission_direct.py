...
class TestFlattenedPermissionDirect:

    def test_should_direct_list_flattened_permission(self):
        setup = _flattened_permission_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-flattened_permission", "live" if setup["live"] else "unit")
        if _skip:
    ...
    def test_should_direct_load_flattened_permission(self):
        setup = _flattened_permission_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-flattened_permission", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
...
def _flattened_permission_direct_setup(mockres):
