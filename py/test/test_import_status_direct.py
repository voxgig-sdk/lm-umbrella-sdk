...
class TestImportStatusDirect:

    def test_should_direct_list_import_status(self):
        setup = _import_status_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-import_status", "live" if setup["live"] else "unit")
        if _skip:
...
def _import_status_direct_setup(mockres):
