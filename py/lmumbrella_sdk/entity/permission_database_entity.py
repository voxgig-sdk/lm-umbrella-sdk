...
from lmumbrella_sdk.lmumbrella_types import (
    PermissionDatabase,
    PermissionDatabaseLoadMatch,
    PermissionDatabaseListMatch,
    PermissionDatabaseUpdateData,
)
...
class PermissionDatabaseEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return PermissionDatabaseEntity(self._client, opts)
    ...
    def data_get(self) -> PermissionDatabase:
    ...
    def match_get(self) -> PermissionDatabase:
    ...
    def load(self, reqmatch=None, ctrl=None) -> PermissionDatabase:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.PermissionDatabase().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[PermissionDatabase]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.PermissionDatabase().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
    ...
    def update(self, reqdata: PermissionDatabaseUpdateData, ctrl=None) -> PermissionDatabase:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
