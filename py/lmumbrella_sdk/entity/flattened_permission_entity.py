...
from lmumbrella_sdk.lmumbrella_types import (
    FlattenedPermission,
    FlattenedPermissionLoadMatch,
    FlattenedPermissionListMatch,
    FlattenedPermissionCreateData,
)
...
class FlattenedPermissionEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return FlattenedPermissionEntity(self._client, opts)
    ...
    def data_get(self) -> FlattenedPermission:
    ...
    def match_get(self) -> FlattenedPermission:
    ...
    def load(self, reqmatch=None, ctrl=None) -> FlattenedPermission:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.FlattenedPermission().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[FlattenedPermission]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.FlattenedPermission().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
    ...
    def create(self, reqdata: FlattenedPermissionCreateData, ctrl=None) -> FlattenedPermission:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
