...
class DatabaseEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return DatabaseEntity(self._client, opts)
    ...
    def remove(self, reqmatch=None, ctrl=None) -> Database:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.Database().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
