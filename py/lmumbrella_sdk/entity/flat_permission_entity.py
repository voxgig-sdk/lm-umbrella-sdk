...
class FlatPermissionEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return FlatPermissionEntity(self._client, opts)
    ...
    def data_get(self) -> FlatPermission:
    ...
    def match_get(self) -> FlatPermission:
    ...
    def load(self, reqmatch=None, ctrl=None) -> FlatPermission:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.FlatPermission().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
