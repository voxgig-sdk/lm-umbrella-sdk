...
class PermissionEntity:
    ...
    def update(self, reqdata: PermissionUpdateData, ctrl=None) -> Permission:
        utility = self._utility
    ...
    def remove(self, reqmatch=None, ctrl=None) -> Permission:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.Permission().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
