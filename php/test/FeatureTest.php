...
class FtCtrl extends LmUmbrellaControl
{
...
class FtClient
{
    ...
    private LmUmbrellaUtility $utility;

    public function __construct(LmUmbrellaUtility $utility, array $options)
    {
    ...
    public function get_utility(): LmUmbrellaUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (LmUmbrellaContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public LmUmbrellaContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (LmUmbrellaContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = LmUmbrellaConfig::make_feature($name);
    ...
    private static function buildUrl(LmUmbrellaSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof LmUmbrellaError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new LmUmbrellaResponse($fetched);
        }
    ...
    private function fail(LmUmbrellaContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (LmUmbrellaContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
