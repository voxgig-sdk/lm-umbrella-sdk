...
class FlatPermissionEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set LMUMBRELLA_TEST_FLAT_PERMISSION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $this->assertNotNull($flat_permission_ref01_data_dt0_loaded);
...
function flat_permission_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("LMUMBRELLA_TEST_FLAT_PERMISSION_ENTID");
