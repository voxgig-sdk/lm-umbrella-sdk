...
class PaginatedPermissionListEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set LMUMBRELLA_TEST_PAGINATED_PERMISSION_LIST_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function paginated_permission_list_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("LMUMBRELLA_TEST_PAGINATED_PERMISSION_LIST_ENTID");
