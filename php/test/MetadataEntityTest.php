...
class MetadataEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set LM_UMBRELLA_TEST_METADATA_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function metadata_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["metadata01", "metadata02", "metadata03", "database01", "database02", "database03"] as $k) {
    ...
    $entid_env_raw = getenv("LM_UMBRELLA_TEST_METADATA_ENTID");
