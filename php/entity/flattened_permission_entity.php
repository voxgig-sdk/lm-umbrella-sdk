...
// LmUmbrella SDK FlattenedPermission entity
...
class FlattenedPermissionEntity
{
    ...
    public function __construct($client, ?array $entopts = null)
    {
        ...
        $this->_name = "flattened_permission";
        $this->_client = $client;
    ...
    {
        $opts = $this->_entopts;
        return new FlattenedPermissionEntity($this->_client, $opts);
    }

    /**
     * @param FlattenedPermission|array $args FlattenedPermission data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = LmUmbrellaHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return FlattenedPermission|array The current FlattenedPermission data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of FlattenedPermission fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = LmUmbrellaHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of FlattenedPermission fields).
     */
    ...
    /**
     * Load a single FlattenedPermission.
     *
     * @param FlattenedPermissionLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed FlattenedPermissionLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return FlattenedPermission|array The loaded FlattenedPermission as an assoc-array at the
     *   SDK boundary; throws LmUmbrellaError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = LmUmbrellaHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
    ...
    /**
     * List FlattenedPermission items matching the given filter.
     *
     * @param FlattenedPermissionListMatch|array|null $reqmatch Match filter (any subset
     *   of FlattenedPermission fields) as an assoc-array; FlattenedPermissionListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return FlattenedPermission[]|array A list of FlattenedPermission items as assoc-arrays at
     *   the SDK boundary; throws LmUmbrellaError on failure (item-5 convention).
     */
    ...
    /**
     * Create a new FlattenedPermission.
     *
     * @param FlattenedPermissionCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed FlattenedPermissionCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return FlattenedPermission|array The created FlattenedPermission as an assoc-array at the
     *   SDK boundary; throws LmUmbrellaError on failure (item-5 convention).
     */
