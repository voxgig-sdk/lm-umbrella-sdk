...
describe("FlatPermissionEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:FlatPermission(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = flat_permission_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"load"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "flat_permission." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set LM_UMBRELLA_TEST_FLAT_PERMISSION_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local flat_permission_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.flat_permission")))
    local flat_permission_ref01_data = nil
    if #flat_permission_ref01_data_raw > 0 then
      flat_permission_ref01_data = helpers.to_map(flat_permission_ref01_data_raw[1][2])
    end
...
function flat_permission_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/flat_permission/FlatPermissionTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read flat_permission test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("LM_UMBRELLA_TEST_FLAT_PERMISSION_ENTID")
