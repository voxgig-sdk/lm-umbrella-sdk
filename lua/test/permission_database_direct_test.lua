-- PermissionDatabase direct test
...
describe("PermissionDatabaseDirect", function()
  it("should direct-list-permission_database", function()
    local setup = permission_database_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-permission_database", setup.live and "live" or "unit")
  ...
  it("should direct-load-permission_database", function()
    local setup = permission_database_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-permission_database", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function permission_database_direct_setup(mockres)
