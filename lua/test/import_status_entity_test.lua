...
describe("ImportStatusEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:ImportStatus(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:ImportStatus(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set LMUMBRELLA_TEST_IMPORT_STATUS_ENTID JSON to run live")
      return
    end
...
function import_status_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("LMUMBRELLA_TEST_IMPORT_STATUS_ENTID")
