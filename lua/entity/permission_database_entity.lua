...
local PermissionDatabaseEntity = {}
PermissionDatabaseEntity.__index = PermissionDatabaseEntity


function PermissionDatabaseEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PermissionDatabaseEntity)
  self._name = "permission_database"
  self._client = client
...
function PermissionDatabaseEntity:get_name()
  return self._name
end


function PermissionDatabaseEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PermissionDatabaseEntity.new(self._client, opts)
end
...
function PermissionDatabaseEntity:mark_deleted()
  self._deleted = true
end


function PermissionDatabaseEntity:deleted()
  return true == self._deleted
end


function PermissionDatabaseEntity:data_set(args)
  if args ~= nil then
...
function PermissionDatabaseEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PermissionDatabaseEntity:match_set(args)
  if args ~= nil then
...
function PermissionDatabaseEntity:match_get()
...
function PermissionDatabaseEntity:stream(action, args, callopts)
...
---@param reqmatch PermissionDatabaseLoadMatch
---@param ctrl? table
---@return PermissionDatabase
---@return string? err
function PermissionDatabaseEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch PermissionDatabaseListMatch
---@param ctrl? table
---@return PermissionDatabase[]
---@return string? err
function PermissionDatabaseEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata PermissionDatabaseUpdateData
---@param ctrl? table
---@return PermissionDatabase
---@return string? err
function PermissionDatabaseEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function PermissionDatabaseEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return PermissionDatabaseEntity
