...
local DatabaseEntity = {}
DatabaseEntity.__index = DatabaseEntity


function DatabaseEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DatabaseEntity)
  self._name = "database"
  self._client = client
...
function DatabaseEntity:get_name()
  return self._name
end


function DatabaseEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DatabaseEntity.new(self._client, opts)
end
...
function DatabaseEntity:mark_deleted()
  self._deleted = true
end


function DatabaseEntity:deleted()
  return true == self._deleted
end


function DatabaseEntity:data_set(args)
  if args ~= nil then
...
function DatabaseEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DatabaseEntity:match_set(args)
  if args ~= nil then
...
function DatabaseEntity:match_get()
...
function DatabaseEntity:stream(action, args, callopts)
...
---@param reqmatch DatabaseRemoveMatch
---@param ctrl? table
---@return Database
---@return string? err
function DatabaseEntity:remove(reqmatch, ctrl)
  local utility = self._utility
...
function DatabaseEntity:_run_op(ctx, post_done)
  local utility = self._utility
