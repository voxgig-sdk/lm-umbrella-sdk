...
boru-driven command-line client **and** interactive REPL for the LmUmbrella
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/lm-umbrella-cli)
make build

# 2. See usage (words, entities, env vars)
./lm-umbrella-cli --help
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/lm-umbrella-cli
   ```
...
Run with no arguments to open a REPL (prompt `lm-umbrella>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `database`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/lm-umbrella-cli`. |
...
(`./lm-umbrella-cli <expr>`) and interactively (the REPL), and expressions compose the
