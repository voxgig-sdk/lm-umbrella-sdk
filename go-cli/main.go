// Command LmUmbrella-cli is a CLI + REPL for the LmUmbrella SDK,
...
const prompt = "lm-umbrella"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "database flat_permission flattened_permission import_status metadata paginated_permission_list permission permission_database"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      LM_UMBRELLA_APIKEY (api key), LM_UMBRELLA_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: LM_UMBRELLA_APIKEY carries the API key and
	// LM_UMBRELLA_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("LM_UMBRELLA_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("LM_UMBRELLA_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewLmUmbrellaSDK(opts)
